...
no_events = 200  # Number of trials
...
gaussx = np.linspace(0,no_events, 100*no_events)
var = no_events*pval*(1-pval)
mean = no_events*pval
gaussy = (np.e**(-(gaussx-no_events*pval)**2/(2*no_events*pval*(1-pval))))/np.sqrt(2*np.pi*no_events*pval*(1-pval))


# Plot the PMF
plt.figure(figsize=(8, 6))
plt.plot(gaussx, gaussy, color = 'black', label = "Theory")
...
plt.legend(loc='best')
plt.grid(True)
plt.savefig("../figs/gausscomp3.png")
plt.show()
'''
# Plot the CDF
plt.figure(figsize=(8, 6))
